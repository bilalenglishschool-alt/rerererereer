...
- API endpoints:
  ...
  - `GET /api/transcribe/jobs` (history, optional filters: `status=queued|processing|done|failed|canceled`, `job_id=<uuid>`)
...
- UI `/transcribe` сохраняет выбранные history filters (`status`, `job_id`) локально в браузере.
