...
8. Для `done/failed/canceled` доступен `DELETE /api/transcribe/jobs/{job_id}`.
9. Для защиты backend применяет upload validation и rate limit (`6 req/min` на IP).
10. Worker периодически чистит старые `done/failed/canceled` transcription jobs и удаляет их файлы.
11. Для UX доступны `GET /api/transcribe/jobs` (history) и `GET /api/transcribe/jobs/{job_id}/transcript`.
...
- `lesson_metrics:queue_latency_ms_last/max/sum/samples:<task_type>`
- `lesson_metrics:processing_duration_ms_last/max/sum/samples`
- `lesson_metrics:processing_duration_ms_last/max/sum/samples:<task_type>`
...
- `/metrics/worker` также включает `dead_letter_requeued_total`, `dead_letter_requeued_last_10m` и `dead_letter_requeued_by_type`
...
- alert endpoint `GET /alerts/worker` (сравнение с threshold из env: errors/dead_letter_requeued_last_10m/dead_letter/queue_depth/transcribe_queue_depth/transcribe_oldest_queue_age/transcribe_oldest_processing_age/transcribe_dead_letter/transcribe_oldest_dead_letter_age/heartbeat_age)
- ops endpoints:
