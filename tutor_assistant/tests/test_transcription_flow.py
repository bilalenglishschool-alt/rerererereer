...
class TranscriptionFlowTest(unittest.TestCase):
    ...
    def test_worker_keeps_canceled_if_canceled_during_processing(self) -> None:
        from unittest.mock import patch
        ...
        def _cancel_job_inside_transcribe(*args, **kwargs):  # noqa: ANN002, ANN003
            with SessionLocal() as db:
                job = db.query(TranscriptionJob).filter(TranscriptionJob.id == UUID(job_id)).first()
                self.assertIsNotNone(job)
                job.status = "canceled"
                job.processing_error = "Canceled by user"
                db.commit()
            return "transcript that must not be finalized"

        with patch(
            "tutor_assistant.worker.transcribe_audio",
            side_effect=_cancel_job_inside_transcribe,
        ):
            process_transcription_job(job_id)
        ...
        with SessionLocal() as db:
            ...
            self.assertIsNotNone(job)
            self.assertEqual(job.status, "canceled")
            self.assertIsNone(job.transcript_text)
